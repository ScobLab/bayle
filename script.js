...
// ====================================================================
// PROMPTS D'ANALYSE (FR et EN, synchronisés avec PROMPT.md)
// ====================================================================
...
const TRANSLATIONS = {
  fr: {
    ...
    demoSectionTitle: "Exemples sans clé API",
    demoCardPrefix: "Analyse",
    demoCardFictif: "Article fictif créé pour la démonstration",
    demoCardCta: "Voir l'analyse →",
    methodeLinkText: "Première fois ? Apprenez à analyser un article →",
    separatorText: "Votre article",
    formSectionTitle: "Analyser un article",
    apiKeySavedText: "✓ Clé API enregistrée",
    apiKeyChangeLink: "Changer / effacer",
    apiKeyLabel: "Clé API",
    apiKeyPlaceholder: "Collez votre clé API Mistral ici",
    clearKeyBtn: "Effacer ma clé",
    tutorialTitle: "Pas encore de clé API ? C'est gratuit et ça prend 1 minute",
    tutorialStep1: 'Rendez-vous sur <a href="https://console.mistral.ai/api-keys" target="_blank" rel="noopener noreferrer">console.mistral.ai/api-keys</a>',
    tutorialStep2: 'Cliquez sur "Créer une nouvelle clé"',
    tutorialStep3: 'Donnez-lui un nom (ex : "Bayle") et cliquez sur "Créer"',
    tutorialStep4: "Copiez immédiatement la clé affichée — elle ne sera plus visible ensuite",
    tutorialStep5: "Collez-la dans le champ ci-dessus",
    tutorialBtn: "Créer ma clé API Mistral gratuitement →",
    articleTextLabel: "Texte de l'article",
    articleTextPlaceholder: "Collez ici le texte intégral de l'article (minimum 200 caractères)…",
    securityNotice: "Votre clé est stockée uniquement dans votre navigateur.",
    analyzeBtn: "Analyser",
    ...
    copyLimites: "Limites",
    copyNiveau: "Niveau",
    errorTooShort: "Texte trop court pour une analyse pertinente (minimum 200 caractères).",
    ...
    errorQuota: "Quota dépassé sur votre compte Mistral. Le tier gratuit a des limites — réessayez dans quelques minutes.",
    errorServer: "Erreur du serveur Mistral, réessayez dans quelques instants.",
    errorGeneric: "Réessayez ou vérifiez votre clé.",
    errorUnreadable: "Réponse de l'API illisible.",
    errorFormat: "Réponse au format inattendu. Vérifiez que votre clé API est bien une clé Mistral valide.",
    errorJSON: "JSON mal formé dans la réponse. Texte brut affiché ci-dessous.",
    warningMissing: "Réponse anormale détectée : rubriques manquantes",
    warningScore: "Réponse anormale détectée : score hors plage [0–10].",
    charCount: "caractère",
    charCountPlural: "caractères"
  },
  en: {
    ...
    demoSectionTitle: "Examples without API key",
    demoCardPrefix: "Analysis",
    demoCardFictif: "Fictional article created for demonstration",
    demoCardCta: "View analysis →",
    methodeLinkText: "First time? Learn how to analyze an article →",
    separatorText: "Your article",
    formSectionTitle: "Analyze an article",
    apiKeySavedText: "✓ API key saved",
    apiKeyChangeLink: "Change / clear",
    apiKeyLabel: "API key",
    apiKeyPlaceholder: "Paste your Mistral API key here",
    clearKeyBtn: "Clear my key",
    tutorialTitle: "No API key yet? It's free and takes 1 minute",
    tutorialStep1: 'Go to <a href="https://console.mistral.ai/api-keys" target="_blank" rel="noopener noreferrer">console.mistral.ai/api-keys</a>',
    tutorialStep2: 'Click "Create new key"',
    tutorialStep3: 'Give it a name (e.g. "Bayle") and click "Create"',
    tutorialStep4: "Copy the key immediately — it won't be shown again",
    tutorialStep5: "Paste it in the field above",
    tutorialBtn: "Create my free Mistral API key →",
    articleTextLabel: "Article text",
    articleTextPlaceholder: "Paste the full article text here (minimum 200 characters)...",
    securityNotice: "Your key is stored only in your browser.",
    analyzeBtn: "Analyze",
    ...
    copyNiveau: "Level",
    errorTooShort: "Text too short for a meaningful analysis (minimum 200 characters).",
    errorNetwork: "Network error. Check your internet connection.",
    errorInvalidKey: "Invalid API key, check at console.mistral.ai",
    errorQuota: "Mistral account quota exceeded. Free tier has limits — try again in a few minutes.",
    errorServer: "Mistral server error, try again shortly.",
    errorGeneric: "Try again or check your key.",
    errorUnreadable: "Unreadable API response.",
    errorFormat: "Unexpected response format. Check that your API key is a valid Mistral key.",
    errorJSON: "Malformed JSON in the response. Raw text displayed below.",
    warningMissing: "Abnormal response detected: missing sections",
    warningScore: "Abnormal response detected: score outside [0–10] range.",
    charCount: "character",
    charCountPlural: "characters"
  }
};
...
function appendDemoCard(data, index) {
  ...
  card.innerHTML = `
    <span class="demo-card-number" data-demo-index="${index}">${t('demoCardPrefix')} ${index}</span>
    <span class="demo-card-title">${escapeHtml(data.titre_demo)}</span>
    <span class="demo-card-fictif" data-i18n="demoCardFictif">${t('demoCardFictif')}</span>
    <span class="demo-card-cta" data-i18n="demoCardCta">${t('demoCardCta')}</span>
  `;
...
function refreshCharCount() {
  ...
  charCount.textContent = `${n} ${n !== 1 ? t('charCountPlural') : t('charCount')}`;
}
...
async function callMistralAPI(apiKey, userText) {
  ...
  if (response.status === 429) {
    throw new Error(t('errorQuota'));
  }
  if (response.status >= 500) {
    throw new Error(t('errorServer'));
  }
  if (!response.ok) {
    throw new Error(`Erreur API (code ${response.status}). ${t('errorGeneric')}`);
  }
...
function showResult(analysis, isDemo, sourceText) {
  lastResult = { analysis, isDemo, sourceText };
  resultZone.innerHTML = '';
  resultZone.style.display = 'block';

  // Bandeau DÉMONSTRATION
  if (isDemo) {
    const banner = el('div', 'demo-banner', t('demoBanner'));
    resultZone.appendChild(banner);
    const fictifNotice = el('p', 'demo-fictif-notice', t('demoFictifNotice'));
  ...
  if (isDemo && sourceText) {
    ...
    summary.textContent = t('demoSourceLabel');
  ...
  if (analysis.fiabilité_globale)
    addSection(t('sectionFiabilite'), buildFiabilite(analysis.fiabilité_globale), true);

  // Sections d'analyse
  if (analysis.locuteur)
    addSection(t('sectionLocuteur'), buildLocuteur(analysis.locuteur));

  if (analysis.faits_vs_opinions)
    addSection(t('sectionFaitsOpinions'), buildFaitsOpinions(analysis.faits_vs_opinions));
  ...
  if (analysis.biais_de_cadrage)
    addSection(t('sectionBiais'), buildBiais(analysis.biais_de_cadrage));

  if (analysis.omissions)
    addSection(t('sectionOmissions'), buildOmissions(analysis.omissions));

  if (analysis.contre_points_légitimes)
    addSection(t('sectionContrePoints'),
  ...
  if (vigilLevel && vigilLevel !== 'aucune' && VIGILANCE_MESSAGES[currentLang]?.[vigilLevel]) {
    ...
    encart.innerHTML = `
      <h3>${escapeHtml(t('vigilanceTitle'))}</h3>
      <p>${escapeHtml(VIGILANCE_MESSAGES[currentLang][vigilLevel])}</p>
      <a href="./risques.html" target="_blank" rel="noopener noreferrer" class="vigilance-link">${escapeHtml(t('vigilanceLink'))}</a>
    `;
...
function buildBiais(b) {
  return buildList(t('labelMotsCharges'), b.mots_chargés, 'list-orange') +
    (b.choix_d_angle
      ? `<div class="list-block"><strong style="font-size:.85rem;">${t('labelAngle')}</strong><p style="font-size:.875rem;margin-top:.25rem;">${escapeHtml(b.choix_d_angle)}</p></div>`
      : '') +
    (b.structure_rhétorique
      ? `<div class="list-block"><strong style="font-size:.85rem;">${t('labelRhetorique')}</strong><p style="font-size:.875rem;margin-top:.25rem;">${escapeHtml(b.structure_rhétorique)}</p></div>`
      : '');
}
...
function buildFiabilite(f) {
  ...
  return `
    ...
    ${f.ce_que_le_lecteur_devrait_creuser
      ? `<div style="margin-top:.75rem;"><strong style="font-size:.85rem;">${t('labelAApprofondir')}</strong><p style="font-size:.875rem;margin-top:.25rem;">${escapeHtml(f.ce_que_le_lecteur_devrait_creuser)}</p></div>`
      : ''}
  `;
}
...
function setLoading(active) {
  ...
  analyzeBtn.textContent = active ? t('analyzingBtn') : t('analyzeBtn');
...
function showRawResponse(rawText) {
  resultZone.style.display = 'block';
  // Safe: rawText passes through escapeHtml()
  resultZone.innerHTML = `
